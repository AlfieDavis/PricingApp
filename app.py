...
authenticator = stauth.Authenticate(
    credentials,
    "coop_pricing_app_5",  # rotate cookie name to avoid stale session
...
def _normalize_upc(raw):
    ...
    if not digits:
        return None
    if len(digits) == 11:
        digits += _upc_check_digit(digits)
...
if authentication_status:
    ...
    st.caption(
        "Upload inputs, optionally edit margins (built from Sales), click Process, then review and export change files."
    )
    ...
    def _prepare_display(df: pd.DataFrame):
        ...
        if "pct_change" in d.columns:
            d["pct_change"] = d["pct_change"] * 100.0
    ...
    with st.sidebar.form("inputs"):
        ...
        sales_file = st.file_uploader(
            "Sales (CSV/XLSX)", type=["csv", "xlsx", "xls"], key="sales"
        )
        sales_sheet = None
        ...
        unfi_file = st.file_uploader(
            "UNFI Bid (CSV/XLSX)", type=["csv", "xlsx", "xls"], key="unfi"
        )
        unfi_sheet = None
        ...
        kehe_file = st.file_uploader(
            "KeHE Price List (XLSX/CSV)", type=["xlsx", "xls", "csv"], key="kehe"
        )
        kehe_sheet = None
        ...
        core_file = st.file_uploader(
            "Core Sets & Basics (XLSX/CSV)", type=["xlsx", "xls", "csv"], key="core"
        )
        core_sheet = None
        ...
        with st.expander("Optional: Department targets override"):
            st.caption(
                "If enabled, build the margin table from departments found in the Sales file (no canonical bucket column)."
            )
            ...
            if allow_edit:
                ...
                if not sales_preview.empty:
                    ...
                    if dept_col:
                        ...
                        depts_df = (
                            tmp[["department_code", "department_name"]]
                            .drop_duplicates()
                            .sort_values(["department_code", "department_name"])
                        )

                if not depts_df.empty:
                    baked = pl.get_baked_targets()[
                        ["department_code", "department_name", "target_margin"]
                    ].copy()
                    targets_base = depts_df.merge(
                        baked, on=["department_code", "department_name"], how="left"
                    )
                else:
                    targets_base = pl.get_baked_targets()[
                        ["department_code", "department_name", "target_margin"]
                    ].copy()
                ...
                targets_override_df = st.data_editor(
                    ...
                    column_config={
                        ...
                        "target_margin": st.column_config.NumberColumn(
                            "Target margin",
                            min_value=0.05,
                            max_value=0.95,
                            step=0.01,
                ...
                st.session_state["targets_template_df"] = pl.get_baked_targets()[
                    ["department_code", "department_name", "target_margin"]
                ].copy()
    ...
    if (
        isinstance(st.session_state.get("targets_template_df"), pd.DataFrame)
        and not st.session_state["targets_template_df"].empty
    ):
            ...
            with st.spinner("Reading files..."):
                ...
                if not sales_df.empty:
                    ...
                    if upc_col and vendor_col:
                        m = (
                            sales_df[[upc_col, vendor_col]]
                            .dropna()
                            .assign(
                                _upc=lambda df: df[upc_col].map(_normalize_upc),
    ...
    if res:
        ...
        a3.metric(
            "GM$ delta (projected − current)", f"${float(summary.get('gm_total_delta', 0) or 0):,.2f}"
        )
        ...
        if (
            "vendor_filter_df" not in st.session_state
            or set(st.session_state["vendor_filter_df"]["Supplier"]) != set(pos_vendor_list)
        ):
            ...
            st.session_state["vendor_filter_df"] = pd.DataFrame(
                {
                    ...
                    "Include": [
                        ("UNFI" in str(v).upper()) or ("KEHE" in str(v).upper())
                        for v in pos_vendor_list
                    ],
                }
            )
        ...
        if view == "All changes":
            ...
            mask = (
                ...
                & diag["CurrentPrice"].notna()
                & diag["new_price"].notna()
        ...
        sel_upcs = (
            set(view_df["upc"].dropna().astype(str)) if not view_df.empty else set()
        )
        ...
        if sel_upcs and not diag.empty:
            gm_delta_view = float(
                diag[
                    diag["upc"].astype(str).isin(sel_upcs)
                    & (~diag["excluded"].fillna(False))
                ]["gm_delta"].sum()
            )
        ...
        st.subheader(
            "Change file (dedup by UPC) — Export this to update POS"
        )
        change_filtered_for_display = _include_actual_suppliers(
            change.copy(), selected_suppliers
        )
        ...
        with left:
            st.subheader(
                "KeHE cheaper than UNFI (both costs present)"
            )
            st.dataframe(
                kehe_cheaper.drop(columns=["canonical_bucket"], errors="ignore"),
                use_container_width=True,
                hide_index=True,
            )
        with right:
            st.subheader(
                "KeHE default but UNFI available (FYI)"
            )
            st.dataframe(
                kehe_default_unfi_available.drop(
                    columns=["canonical_bucket"], errors="ignore"
                ),
                use_container_width=True,
                hide_index=True,
            )
        ...
        st.download_button(
            "Download change file (FULL)",
            _bytes_csv(change),
            file_name=change_name,
            mime="text/csv",
        )
        st.download_button(
            "Download diagnostics (all rows)",
            _bytes_csv(diag),
            file_name=diag_name,
            mime="text/csv",
        )
        st.download_button(
            "Download GM$ delta by department",
            _bytes_csv(gm_summary),
            file_name=gm_name,
            mime="text/csv",
        )
        ...
        st.info(
            "Choose files in the sidebar, pick sheets if applicable, optionally adjust department targets, then click **Process**."
        )
